  ...
  return (
    <div className='wrapper'>
      ...
      <div className='sonnets-container'>
        ...
        {searchInput && filteredSonnets.map(sonnet => (
          <div key={sonnet.number} className='sonnet'>
            <h3>{`Sonnet ${sonnet.number}`}</h3>
            {sonnet.lines.map((line, index) => {
              const highlightedLine = line.split(new RegExp(`(${searchInput})`, 'gi')).map((part, id) =>
                part.toLowerCase() === searchInput.toLowerCase() ? (
                  <span key={id} className='highlight'>{part}</span>
                ) : (
                  part
                )
              );
              return <p key={index}>{highlightedLine}</p>;
            })}
          </div>
        ))}
      </div>
